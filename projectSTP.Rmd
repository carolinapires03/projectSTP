...
However, we still need to compare the models based on the quality of their forecasts.
```{r}
library(forecast)
```
...
m22_arima <- Arima(ly, order = c(3, 0, 0), seasonal=list(order=c(1,1,0), period=12), include.constant = FALSE)
m23_arima <- Arima(ly, order = c(2, 0, 1), seasonal=list(order=c(1,1,0), period=12), include.constant = FALSE)

hmax <- 23
m21.f.h <- forecast(m21_arima, h=hmax)
m22.f.h <- forecast(m22_arima, h=hmax)
m23.f.h <- forecast(m23_arima, h=hmax)
```
...
m22.f.h.acc = accuracy(m22.f.h, log_test)
m23.f.h.acc = accuracy(m23.f.h, log_test)
m21.f.h.acc
```
```{r}
m22.f.h.acc
...
m23.f.h.acc
```
The 3 models performed similarly on the training set, but m21 shows a clear advantage on the test set results in all the most relevant metrics (RMSE, MAE, MAPE, MASE).
...
comparison_table <- data.frame(
  Date = seq(as.Date("2016-01-01"), by = "month", length.out = length(test_values)),  # Adjust if needed
  Actual = test_values,
...
evaluation_df <- data.frame(
  Metric = c("MAE", "RMSE", "MAPE", "MedAE", "R-Squared", "Bias"),
  Value = c(mae, rmse, mape, medae, r_squared, bias)
)
...
CROSS-VALIDATION 23 months with 1-step ahead
...
for (i in 1:length(test_mato_grosso)) {
  train_set <- ts_mato_grosso_shifted[1:(train_size + i - 1)]
  test_set <- ts_mato_grosso_shifted[(train_size + i):(train_size + i)]
  ...
  lower_bound <- forecast_result$lower[1, 2]
  upper_bound <- forecast_result$upper[1, 2]
...
forecast_df <- data.frame(
  ...
  Error = test_values - forecast_values,  # Error: Actual - Forecast
  Absolute_Error = abs(test_values - forecast_values),  # Absolute Error
  Percentage_Error = (abs(test_values - forecast_values) / test_values) * 100  
)
...
# Benefits of the SARIMA Model

#Captures Seasonality:

The SARIMA (2,0,0)(1,1,0)[12] configuration effectively accounts for the 12-month seasonal cycle present in forest fire data, which is crucial for understanding and predicting annual fire trends.

#Handles Trends and Differencing:

The differencing components (D=1) help handle non-stationarity in the seasonal cycle, which is common in long-term time series like forest fires.

#Good Fit Without Cross-Validation:

Metrics like R^2 = 0.64 and RMSE = 2973 (no cross-validation experiment) show that SARIMA can reasonably fit the training data, capturing significant patterns and trends.

#Data-Driven Approach:

SARIMA models are well-suited for data with clear seasonality and autocorrelation, making them applicable to natural phenomena like forest fires.

#-------------------------------------------------------------------

#Limitations of SARIMA for Forest Fire Data

#Struggles with Extreme Variability:

Forest fires exhibit high variability due to external factors (e.g., rainfall, temperature, policy changes). SARIMA cannot incorporate these external drivers, limiting its ability to model extreme changes.

# Performance Drops with Cross-Validation:

Metrics like RMSE = 5417 and R^2 = −0.19 in cross-validation highlight poor generalization. This reflects:
Smaller training subsets in time-aware cross-validation lead to less effective learning of seasonal and trend components.
Temporal variability (e.g., long-term trends due to climate change) reduces the model's ability to generalize.

# Assumes Stationarity:

SARIMA assumes stationarity in seasonality and trends. If the underlying patterns shift over time, the model may become outdated or inaccurate.

# No External Predictors:

SARIMA is limited to past values of the target variable. It cannot consider external factors (e.g., El Niño, deforestation rates), which are critical for forest fire dynamics.

# Wide Prediction Intervals:

The model often produces broad 95% confidence intervals, indicating high uncertainty in forecasts, particularly for extreme values or unseen test data.


#-------------------------------------------------------------------

# Insights from Time Series Cross-Validation

# Time-Aware Validation:

Cross-validation for time series respects temporal order, using past data to predict future observations. This better reflects real-world forecasting scenarios.
However, metrics like MAE = 2704 and RMSE = 5417 indicate the model struggles with unseen data, particularly in recent years where variability may be higher.

#Reasons for Performance Drop:

Temporal Shifts: Cross-validation highlights that patterns in the training set may not fully align with those in the test set (e.g., due to climate or policy changes).

Edge Effects: SARIMA depends on lagged values, and forecasts near the train-test boundary can be less reliable.
